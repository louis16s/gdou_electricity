...
# 楼栋ID
budingID = 'enter your own buildingID'
# 电表地址
comAddress= "enter your own comAddress"
...
def send_push_notification(content):
    if bark_token == '':
        ...
        url = "https://api.day.app/"+bark_token+'/' + content  # Replace the content after the slash with your own notification content
...
if __name__ == '__main__':
    if datas == []:
        ...
        send_request(url)
        if datas == []:
            send_push_notification('电费查询失败')
            exit()
        else:
            datas[8] = datas[8].replace('-','');datas[9] = datas[9].split(':',1)[1]
            print(datas[3]);print(datas[6]);print(datas[8]);print(datas[9])
